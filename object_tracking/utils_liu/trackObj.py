...
from utils_liu import Kalman, Datastruc, locateObj

'''
                                Note: 4 predefined STATUS
                Movement Detected?  Previous Movement Successive?   BinaryCode      Status Number
    backgroud           No                      No                      (0,0)           0
...
'''
                                        Monitor
The monitor in Tracker saves the observed centers of the object in several previous frames. When the status becomes
2:occlusion, the monitor will predict the path of the object based on the data it collected before. One should set the
prediction method. Now there are 3 methods predefined. It's clear that we shall not work directly on the center coordinates,
...
mean value of the data. 'Gaussian' is much more funny. It assumes that the speed and acceration of the object is subjected to
Gaussian distribution, for which when the object is under occlusion, the monitor will guess the new speed randomly.
'''
'''
...
- fps: (float) frame per second, a metric to measure the speed of the object in current frame
...
- thresh: (1x2 tuple) the minimum number of the previous successive target movement
...
class Tracker(object):
    ...
    def update(self,center,torr):
        ...
        objOcc, self.objCounter = self.judge(self.objCounter,self.thresh[0])
    ...
    # here define a function to judge if object has been blocked
    def judge(self,counter,thresh):
        isOcc = counter>=thresh
        if not max(self.center):
            counter = 0
        return isOcc, counter
    
    ########################
    # Here, define several abstractmethod and just use pass in each function 
    # Therefore, different child class can rewrite these methdos accordingly
    # super class's instance cannot use these methods
    ########################
    @abstractmethod
    def ignore(self):
        '''
...
'''
# DenseTracker succeed Tracker class
class DenseTracker(Tracker):
    ...
    def draw(self, color):
        # draw a rectangle
        # pt1 and pt2 are opposite corner points of a rectangle and pt1 is at left-top corner, pt2 is art right-bottom corner
        # However, what should be noticed is in the image showed by openCV, y value is horizontal while x value is vertical
        # This setting is difference to the numpy's matrix operation 
    ...
    def loop(self, idx1, idx2, Filter=False):
        ...
        for i in range(idx1, idx2, self.step):
            ...
            self.im = im2
            roi, center, index = locateObj.conv(im1, im2, self.kernel, self.stride, index=index)
            if Filter:
...
# here define a SparseTracker class that succeed DenseTracker class
class SparseTracker(DenseTracker):
    ...
    def loop(self, idx1, idx2, Filter=False):
        ...
        for i in range(idx1, idx2, self.step):
            ...
            print(status)
            if status is 3:
                feat,center,roi = locateObj.track(im1, im2, roi, self.fn, feat, self.kernel)
            else:
                roi, center, index = locateObj.conv(im1, im2, self.kernel, self.stride, index=index)
            if Filter:
                center = self.filter.filt(center)
